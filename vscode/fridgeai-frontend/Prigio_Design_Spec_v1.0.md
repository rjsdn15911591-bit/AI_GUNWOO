...
#### 일괄 선택 삭제 (Multi-select Delete) — 디자인 의도

| 구성 요소 | 스펙 |
|-----------|------|
| 진입 버튼 | 헤더 우측 "선택 삭제" — Warm Amber `#FAC775` 텍스트 + Amber border / 재료가 1개 이상일 때만 표시 |
| 선택 모드 툴바 | 헤더 아래 고정 배너 / bg `rgba(250,199,117,0.08)` + border-bottom Amber / "N개 선택됨" + "전체 선택" + 삭제 버튼 |
| 삭제 버튼 | bg `#C0392B` (Danger) / Disabled(선택 0개) opacity 0.4 / "🗑 삭제 (N개)" |
| 체크박스 | 각 재료 행 좌측 / 20×20px / 선택 시 bg Prigio Green + 흰 체크마크 SVG / 미선택 시 border `#D3D1C7` |
| 행 선택 하이라이트 | 선택된 행: bg `rgba(29,158,117,0.08)` + left border 3px Green |
| 취소 | 헤더 좌측 "취소" 버튼 (평상시 "← 대시보드" 자리) — Mint 텍스트 |

> **디자인 의도**: 삭제 동작은 비가역적이므로 실수 방지를 위해 "선택 삭제" 버튼(Amber)을 헤더에 분리 배치하고, 선택 모드 진입 후에만 삭제 버튼이 활성화됩니다. 선택 모드 전체가 Amber 계열로 통일되어 "경고·주의" 맥락임을 색상으로 암시합니다. Danger Red(`#C0392B`) 삭제 버튼은 선택 항목이 있을 때만 활성화되어 빈 삭제 요청을 원천 차단합니다.
...
> **영양 정보 2×2 그리드 디자인 의도**: 영양 정보는 보조적 참고 데이터이므로 주요 콘텐츠(재료·조리법)를 방해하지 않도록 가장 하단에 배치합니다. 각 영양소를 동일한 비중의 카드로 나열하면 스캔 속도가 빠르고, Ice 계열 배경(`#F5F3EE`)은 본문 카드(White)와 구분되어 "부록" 섹션임을 직관적으로 전달합니다.
